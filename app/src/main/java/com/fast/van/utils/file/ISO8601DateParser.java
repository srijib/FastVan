...
public class ISO8601DateParser {
    ...
    public static Date parse(String date) throws ParseException {
        ...
        switch (buffer.length()) {
            ...
            default:
                ...
                if (buffer.length() < 16) {
                    throw new ParseException("Invalid ISO 8601 date: " + buffer, buffer.length());
                }
        ...
        /*Date parsedDate = format.parse(buffer.toString());
        
        // Added this hack to fix Android bug where it doesn't account for Daylight Savings Time properly when calculating dates from GMT
        // https://code.google.com/p/android/issues/detail?id=26667
        if (isDST) {
        	Calendar tmpCal = Calendar.getInstance();
        	tmpCal.setTime(parsedDate);
        	tmpCal.add(Calendar.HOUR_OF_DAY, 1);
        	parsedDate = tmpCal.getTime();
        }*/
    ...
    /* Added for program compatibility */
